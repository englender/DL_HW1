...
class LinearClassifier(object):
    ...
    def __init__(self, n_features, n_classes, weight_std=0.001):
        ...
        self.weights = None
        # ====== YOUR CODE: ======
        t1 = torch.randn((n_features, n_classes))
        t1 *= weight_std
        self.weights = t1
        # ========================
    ...
    def train(self,
        ...
        for epoch_idx in range(max_epochs):
            ...
            for x_train, y_train in dl_train:
                ...
                loss = loss_fn.loss(x_train, y_train, x_scores, y_pred) + (weight_decay/2)*(self.weights.norm() ** 2)
                average_loss += loss
                count += 1
                total_correct += self.evaluate_accuracy(y_train, y_pred)*x_train.size()[0]/100
                self.weights = (loss_fn.grad() + weight_decay * self.weights)*(-1)*learn_rate + self.weights
            train_res.accuracy.append(total_correct/len(dl_train))
            train_res.loss.append(average_loss/count)
            count, total_correct, average_loss = 0, 0, 0

            for x_valid, y_valid in dl_valid:
                y_pred, x_scores = self.predict(x_valid)
                loss = loss_fn.loss(x_valid, y_valid, x_scores, y_pred) + (weight_decay/2)*(self.weights.norm() ** 2)
                average_loss += loss
                count += 1
            ...
            valid_res.loss.append(average_loss / count)
            # ========================
            print('.', end='')
    ...
    def weights_as_images(self, img_shape, has_bias=True):
        ...
        img_shape = (self.n_classes, *img_shape)
        if has_bias:
            w_images = self.weights[:-1, :]
        w_images = w_images.t().view(img_shape)
        print(w_images.size())
        # ========================
