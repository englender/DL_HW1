...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # ====== YOUR CODE: ======
        coef = 1/len(y)
        lambda_id = self.reg_lambda*np.identity(X.shape[1])
        lambda_id[0, 0] = 0
        mat = coef*(X.transpose().dot(X)) + lambda_id
        mat = np.linalg.inv(mat)
        w_opt = coef*mat.dot((X.transpose().dot(y)))
        # ========================
        self.weights_ = w_opt
        return self
...
class BostonFeaturesTransformer(BaseEstimator, TransformerMixin):
    ...
    """
    def __init__(self, degree=2, add_log_inicies=None):
        self.degree = degree

        # TODO: Your custom initialization, if needed
        # Add any hyperparameters you need and save them as above
        # ====== YOUR CODE: ======
        self.log_i = add_log_inicies
        # ========================
    ...
    def transform(self, X):
        ...
        a = X[:, [0, 3, 6, 10, 11, 13]]
        X_transformed = a
        X_transformed = poly.fit_transform(X_transformed[:, 1:])
        # ========================
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    top_n = feature_corr.abs().sort_values(ascending=False)[1:n+1].index
    top_n_features = []
    top_n_corr = []
    for feat in top_n:
        top_n_features.append(feat)
        top_n_corr.append(feature_corr[feat])
    # ========================
...
def cv_best_hyperparams(model: BaseEstimator, X, y, k_folds,
    ...
    # ====== YOUR CODE: ======
    dict = {'bostonfeaturestransformer__degree': degree_range, 'linearregressor__reg_lambda': lambda_range}
    scorer = sklearn.metrics.make_scorer(sklearn.metrics.mean_squared_error, greater_is_better=False)
    cv = sklearn.model_selection.GridSearchCV(model, dict, scorer, cv=k_folds)
    cv.fit(X, y)
